...
for j in range(n_sets):
    ...
    # Profitti attesi e tempi per diversi k
    profits_k = []
    times_k = []
    times_k_solve = []
    ...
    print(f"\nSet {j+1:2d}: profitto atteso medio (K={k_min}-{k_max}) = {set_mean:.2f}€, tempo medio riduzione = {set_mean_time:.4f}s")
...
print("\n============================")
print(f"Media dei profitti medi sui 20 set: {mean_overall:.2f}€")
print(f"Deviazione standard dei profitti medi: {std_overall:.2f}€")
...
for j in range(n_sets):
    ...
    for k in range(k_min, k_max+1):
        start = time.perf_counter()
        demands_reduced, probs_reduced = scen_tree.reduce_scenarios_wasserstein_1D(np.array(demands_agg), probs_agg, k=k)
        end = time.perf_counter()
        times_k.append(end - start)

        # Stampa scenari ridotti come da tuo esempio
        print(f"\n📉 Scenari ridotti via Wasserstein distance (set {j+1}, k={k}):")
        for d, p in zip(demands_reduced, probs_reduced):
            print(f"d = {d}, π = {p:.2f}")
        print(f"🔎 Somma delle probabilità: {sum(probs_reduced):.2f}")
